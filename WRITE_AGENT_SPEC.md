...
|---|---|---|---|---|---|---|---|
| M0 方案落地与骨架搭建 | TODO | TBA |  |  | [ ] ADR 完成并落盘；<br>[ ] 模块目录骨架完成；<br>[ ] migration/schema 初版完成；<br>[ ] feature flags 生效验证 |  |  |
| M1 Workspace 最小模型 | TODO | TBA |  |  | [ ] users/workspaces/workspace_members 建表；<br>[ ] 登录后无 Workspace 引导创建；<br>[ ] 聊天页绑定当前 Workspace；<br>[ ] Workspace 鉴权校验通过 |  |  |
| M2 Table Catalog 能力 | TODO | TBA |  |  | [ ] table_catalog 建表；<br>[ ] Catalog CRUD API 完成；<br>[ ] catalog service 能按 business_type 命中 active target；<br>[ ] 前端最小可见 |  |  |
| M3 Upload Inspection 新链路 | TODO | TBA |  |  | [ ] `/ingestion/uploads` 完成；<br>[ ] 原始文件落盘；<br>[ ] sheet/column/sample/hash 提取；<br>[ ] ingestion_uploads/ingestion_jobs 持久化 |  |  |
| M4 Write Ingestion Agent Runtime | TODO | TBA |  |  | [ ] WriteIngestionAgentRuntime 完成；<br>[ ] tools 挂载完成；<br>[ ] structured output schema 完成；<br>[ ] proposal 持久化；<br>[ ] Query/Write 分流稳定 |  |  |
| M5 Setup Flow | TODO | TBA |  |  | [ ] setup trigger 完成；<br>[ ] setup schema 完成；<br>[ ] setup 卡片 UI 完成；<br>[ ] setup confirm API 完成；<br>[ ] catalog entry 自动生效 |  |  |
| M6 Approval + Execution | TODO | TBA |  |  | [ ] `/ingestion/approve` 完成；<br>[ ] `/ingestion/execute` 完成；<br>[ ] SQL validator 完成；<br>[ ] dry-run summary 可用；<br>[ ] execution+receipt 持久化 |  |  |
| M7 前端完整体验 | TODO | TBA |  |  | [ ] 上传后状态机完成；<br>[ ] setup/proposal/receipt 卡片全链路完成；<br>[ ] Workspace 绑定展示完成；<br>[ ] 错误态可解释 |  |  |
