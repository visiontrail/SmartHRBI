...
Claude Agent SDK 官方支持 **自定义工具、MCP、hooks、structured outputs、权限控制、subagents、用户输入/审批回调** 等能力；本项目在审批触发上统一使用 `AskUserQuestion`，但仍保持 **“Agent 输出结构化 proposal + 前端审批卡片 + 后端确定性执行”** 的产品约束，避免把业务写入确认完全托管给 SDK 的自由交互。
...
## 10.5 `AskUserQuestion` 审批约束

Claude SDK 的 `AskUserQuestion` 作为审批提问入口，必须受产品约束使用，不能变成任意自由问答。约束如下：

- 仅允许两个阶段：
  - `catalog_setup`
  - `proposal_approval`
- `catalog_setup` 选项固定为 `confirm_catalog_setup` / `cancel`
- `proposal_approval` 选项固定为 `update_existing` / `time_partitioned_new_table` / `new_table` / `cancel`
- Agent 必须在返回最终 structured output 之前调用 `AskUserQuestion`
- 业务审批的生效仍以 `/ingestion/approve` 为准，`AskUserQuestion` 只负责向前端触发审批问题展示
...
8. SDK 的 `AskUserQuestion` 作为正式审批触发机制使用，但选项与阶段必须严格受控，不能引入自由文本审批。
